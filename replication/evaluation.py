...
for run in range(num_runs):
        ...
        wandb.log(
            {
                ...
                "test_rmse": test_error[0],
                "test_mae": test_error[1],
            }
        )
        wandb.finish()
    ...
    if key in keys:
        ...
        reg_model = create_baseline_nn_model(
            best_params["h_l"],
            best_params["lr"],
    ...
    if key in keys:
        y_tv_dist, y_test_dist, centers = transform_normal(
            y_tv,
            y_test,
            y_min,
            y_max,
            n_bins=best_params["n_bins"],
            ker_par_ratio=best_params["ker_par"],
        )
        ...
        history[key].append(
            cat_model.fit(
                X_tv,
                y_tv_dist,
                validation_data=(X_test, y_test_dist),
                epochs=epochs,
                batch_size=best_params["batch_size"],
                verbose=verbose,
                callbacks=[WandbMetricsLogger()],
