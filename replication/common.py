...
def create_baseline_nn_model(
    ...
    if clipping is None:
        model.compile(
            loss=loss, optimizer=Adam(lr=lr, epsilon=adam_eps, amsgrad=use_amsgrad)
        )
    else:
        model.compile(
            loss=loss,
            optimizer=Adam(
                lr=lr, epsilon=adam_eps, amsgrad=use_amsgrad, clipnorm=clipping
            ),
        )
    return model
...
def transform_normal(y_tv, y_test, y_min, y_max, n_bins=100, ker_par_ratio=1.0):
    ...
    # Distribution
    border_targets_tv = adjust_and_erf(
        borders[np.newaxis, :], y_tv[:, np.newaxis], ker_par
    )
    two_z_tv = border_targets_tv[:, -1] - border_targets_tv[:, 0]
    y_tv_dist = (border_targets_tv[:, 1:] - border_targets_tv[:, :-1]) / two_z_tv[
        :, np.newaxis
    ]

    border_targets_test = adjust_and_erf(
        borders[np.newaxis, :], y_test[:, np.newaxis], ker_par
    )
    two_z_test = border_targets_test[:, -1] - border_targets_test[:, 0]
    y_test_dist = (
        border_targets_test[:, 1:] - border_targets_test[:, :-1]
    ) / two_z_test[:, np.newaxis]
...
def get_data(dataset):
    ...
    if dataset == "yearpred":
        ...
        dat = pd.read_csv(os.path.join("data", "slice_localization_data.csv")).iloc[
            :, 1:
        ]
        y_min = 0
        y_max = 100
...
def get_split(dat, dataset, test_ratio=0.2, seed=42):
    ...
    if dataset == "yearpred_fixed":
        ...
        X_tv, X_test, y_tv, y_test = train_test_split(
            dat.iloc[:, 1:], dat.iloc[:, 0], test_size=test_ratio, random_state=seed
        )
    else:  # CT Scan
        X_tv, X_test, y_tv, y_test = train_test_split(
            dat.iloc[:, :-1], dat.iloc[:, -1], test_size=test_ratio, random_state=seed
        )
